...
function autocompleteSource(e, t) {
    var r;
    function n(e) {
        if (!e)
            return !1;
    ...
    e.addEventListener("input", function(n) {
        var o, i, l, c = this.value;
        if (s(),
        !c)
            return !1;
        for (r = -1,
        (o = document.createElement("DIV")).setAttribute("id", this.id + "autocomplete-list"),
        o.setAttribute("class", "autocomplete-items"),
        this.parentNode.appendChild(o),
        l = 0; l < t.length; l++)
            t[l].toUpperCase().includes(c.toUpperCase()) && ((i = document.createElement("DIV")).innerHTML = "<strong>" + t[l].substr(0, c.length) + "</strong>",
            i.innerHTML += t[l].substr(c.length),
            i.innerHTML += "<input type='hidden' value='" + t[l] + "'>",
            i.addEventListener("click", function(t) {
                e.value = this.getElementsByTagName("input")[0].value,
                finalData.journey_details.from = this.getElementsByTagName("input")[0].value,
                s()
            }),
            o.appendChild(i))
    }),
...
function autocompleteDestination(e, t) {
    var r;
    function n(e) {
        if (!e)
            return !1;
    ...
    e.addEventListener("input", function(n) {
        ...
        l = 0; l < t.length; l++)
            ...
            i.addEventListener("click", function(t) {
                e.value = this.getElementsByTagName("input")[0].value,
                finalData.journey_details.destination = this.getElementsByTagName("input")[0].value,
                s()
            }),
            o.appendChild(i))
    }),
...
fetch("stationlist.json").then(e => e.json()).then(e => {
    ...
    autocompleteSource(document.getElementById("from-station-input"), stationList),
    autocompleteDestination(document.getElementById("destination-station-input"), stationList)
}
),
window.addEventListener("load", () => {
    ...
    document.querySelector("#boarding-station-input").addEventListener("change", setBoardingStation),
    ...
    document.querySelector("#train-no").addEventListener("change", setTrainNumber);
    for (let e = 0; e < 4; e++)
